...
import { LayoutDashboard, Users, Activity, BarChart3, Calendar, FileText, Target, Award, Video, Moon, Sun, Menu, X, Shield, Brain, BookOpen, Layers } from 'lucide-react';
...
const nav = [
  ...
  { href: '/dashboard/principles', label: 'Principles', icon: BookOpen },
...
export default function Sidebar() {
  ...
  const allNav = isAdmin ? [...nav, { href: '/dashboard/admin', label: 'Admin Panel', icon: Shield }] : nav;
  ...
  const inner = (
    <>
        ...
        {allNav.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)}
              className={cn('flex items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] font-semibold transition',
                active ? (adm ? 'bg-red-600 text-white' : 'gradient-brand text-white shadow-glow-orange') : 'hover:bg-brand-50 dark:hover:bg-slate-700',
                adm && !active && 'text-red-600'
              )}
              style={{ color: active || adm ? undefined : 'var(--text-secondary)' }}
            ><Icon size={16} /> {item.label}</Link>
          );
        })}
      </nav>
      <div className="p-2 border-t" style={{ borderColor: 'var(--border)' }}>
        <button onClick={toggleDark} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-[13px] font-semibold transition hover:bg-brand-50 dark:hover:bg-slate-700"
      ...
      <div className="px-3 py-2 text-[10px] text-center border-t" style={{ borderColor: 'var(--border)', color: 'var(--text-secondary)' }}>
        <span className="font-bold" style={{ color: 'var(--text-primary)' }}>Ajman Coach Pro</span> v3
        {isAdmin && <span className="ml-1 text-[9px] px-1.5 py-0.5 rounded-full bg-red-100 text-red-700 font-bold">ADMIN</span>}
      </div>
    </>
  );
    ...
    <>
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          <div className="w-64 card rounded-none flex flex-col border-r shadow-xl" style={{ borderColor: 'var(--border)' }}>
      ...
      <aside className="w-60 card rounded-none border-r border-y-0 border-l-0 flex-shrink-0 hidden md:flex flex-col" style={{ borderColor: 'var(--border)' }}>
        {inner}
      </aside>
    </>
  );
